...
func NewAgent(store *Store, openai *OpenAIClient, retriever *Retriever) *Agent {
	return &Agent{
		store:     store,
		openai:    openai,
		retriever: retriever,
...
If no tool is needed, respond normally.`
	...
	if a.retriever != nil && a.retriever.kb.ChunkCount() > 0 {
		ragContext, err := a.retriever.BuildContext(goal)
		if err != nil {
			fmt.Printf("[rag] retrieval warning: %v\n", err)
		} else if ragContext != "" {
			instructions += "\n\nUse the following retrieved context to inform your answer. " +
				"If the context is relevant, incorporate it. If not, ignore it.\n\n" + ragContext
		}
	}
